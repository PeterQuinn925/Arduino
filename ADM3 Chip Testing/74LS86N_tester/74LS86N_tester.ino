// ============================================================
//  74L862N 4x 2-Input XOR Gate Tester
//  Arduino Nano
...
//   Gate 3 inputs : A2(3A)[9], D8(3B)[10],  output: D9(3Y)[8]
//   Gate 4 inputs : D10(4A)[12], A0(4B)[13],  output: D4(4Y)[11]
//   VCC[14]  -> 5V  |  GND[7] -> GND
//
...
bool expectedOR(uint8_t combo) {
  return (combo != 0);
}
bool expectedXOR(uint8_t combo) {
...
bool testGate(int gateNum,
  ...
  for (uint8_t combo = 0; combo < 4; combo++) {
    ...
    int expected = expectedXOR(combo) ? 1 : 0;
...
void setup() {
  ...
  Serial.println(F("\n========================================"));
  Serial.println(F("  74LS86N 4x XOR Gate Tester"));
  Serial.println(F("========================================"));
