...
I want to build a collar that tracks his position and puts the locations on a map. He always comes in after a few hours so there’s no need to go find him. There’s also no need for real time tracking.
...
Getting low power/deep sleep to work is pretty straightforward. It uses the regular ESP32 deep sleep commands. You just set the amount of time to sleep and when you reach the part of the code when it's time to sleep, you just call the deep sleep command. After the desired amount of time, it essentially reboots.
After optimizations, battery now lasts 4-6 hrs. I considered swapping out the battery for a slightly larger one for more run time, but the size and weight is already on the edge of too much.
I also tried to get it to detect battery vs USB power. Goal here was to show the battery fill level, connection stats, if it’s backfilling, etc when the device is plugged in. And not have it sleep when plugged in. Spent the better part of a day doing this before figuring it out I would need to do it with additional hardware. Granted, it would be a couple of resistors and some soldering, but it wasn’t worth the effort.
## 3D printing a case
I designed the case in OpenSCAD. It's an open source CAD modeling tool where you write code to create objects rather than draw them with a mouse. For a simple part this this, it's pretty easy. I designed it in two parts - the main body and a simple top to be screwed on. After printing, the hole for the USB C connector wasn't quite big enough. The USB spec says it's 8.3mm wide and the hole in the cad model is 9mm so the print must have shrunk a small amount since it doesn't quite fit. I enlarged it slightly with a dremel. Getting it to fit so it could charge was a little bit of a challenge.
Thank you Mana https://manadream.shop/ for 3D printing the case.

Also, I didn't do a great job planning for the fastening the top. Initially I thought I would just have it printed with no screw holes and drill them later. The guy who did the printing for me suggested adding holes in the design as drilling after the fact might ruin it. So I added 2mm holes, thinking I would use M2 threaded inserts. When I got the case, I went to the local hardware store to see what they had that would work. The smallest imperial sizes were way too big. They had three or four sizes of M2 screws but even these were too big for the holes. They did have one size of M1.6 screws that I bought. I figured for 50¢s a piece I'd give it a try before ordering some online. They appear to work fine. If I print it again, I'm making the holes a little bigger so I can use M2 size screws.

I bought a cat collar at the local pet store. I cut the stitching for the collar to remove one end of the clasp. After feeding it through the ends of the case and back through the clasp, I reconnected it with a dollop of hot glue. I may replace the hot glue with epoxy if turns out to be a weak spot.
